...
ProxError_t windows_main() {
    filePath = strcat(getenv("TMP"), fileName);
...
mumble_error_t mumble_init(mumble_plugin_id_t pluginID) {
    ...
    #ifdef _WIN32
    strycpy(logPath, getenv("HOME")); // Change to windows version
    strcat(logPath, ""); // make final path point to \Users\MyGuy\AppData\Local\Mumble\tModLoaderProxChat.log
    #else
